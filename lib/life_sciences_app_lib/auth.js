...
export function logout(router) {
  ...
  if (isBrowser()) {
    try {
      ...
      const returnUrl = `${CONFIG.logoutUri}?force=true`;

      // Cognito logout endpoint - note: logout_uri must be registered in App Client settings
      const cognitoLogoutUrl = `${CONFIG.cognitoDomain}/logout?` +
        `client_id=${CONFIG.clientId}&` +
        `logout_uri=${encodeURIComponent(returnUrl)}`;
